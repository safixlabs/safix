...
/// What the protocol looks like when something is wrong. Each of these has an action attached in
/// monitor/README.md; an alert with no action is noise that teaches people to ignore the channel.
export type AlertKind =
  /// The pool is lent out past the point where it can absorb a liquidation and let providers leave.
  | "utilisation_high"
  /// A position has been liquidatable for longer than it should take to clear one.
  | "position_at_risk"
  /// A position has been liquidatable long enough that the problem is not the position.
  | "keeper_silent"
  /// A price is close to, or past, the age at which the pool will stop acting on it.
  | "stale_price"
  /// A single deposit or withdrawal large enough to move the pool on its own.
  | "large_flow"
  /// The monitor itself could not complete a pass.
  | "scan_failed"
...
export function createAlerter(webhookUrl: string | null, cooldownMs: number, instanceId: string): Alerter {
  ...
  async function fire(alert: Alert) {
    ...
    const details = Object.entries(alert.fields ?? {})
      ...
      .join("\n")
    const body = {
      content: `${emoji} **safix monitor / ${instanceId}** — ${alert.kind}\n${alert.message}${details ? "\n```\n" + details + "\n```" : ""}`,
      text: `${emoji} safix monitor / ${instanceId} — ${alert.kind}: ${alert.message}${details ? "\n" + details : ""}`
    }
...
/// Formats a stable amount with six decimals for a human reading an alert at three in the morning.
export function usd(amount: bigint): string {
  const whole = amount / 1_000_000n
  return `${whole.toLocaleString("en-US")} USDG`
}

/// Basis points as a percentage, for the same reader.
