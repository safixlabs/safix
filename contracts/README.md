...
**The sequencer.** Robinhood Chain is an Arbitrum Orbit rollup. A price feed can keep returning a recent-looking answer while the sequencer is down, and can return a perfectly good one the instant it comes back, before any borrower has had a chance to react. `setSequencerUptimeFeed(feed, gracePeriod)` wires Chainlink's L2 sequencer uptime feed: answer `0` is up, `1` is down, and `startedAt` is when that last changed. Prices are refused while the sequencer is down and for `gracePeriod` after it returns. A feed that reverts, or one whose round never started, counts as down. **Chainlink has not published an uptime feed for Robinhood Chain, and is no longer adding them to new networks.** Until one exists the address is left unset, which means no sequencer check — the behaviour the pool had before. When one is published, wiring it is a single owner transaction and needs no redeploy. `scripts/check-sequencer-feed.mjs` asks Chainlink again every Monday — its machine-readable directory for chain 4663 and its documentation page, each checked first against Arbitrum One's published feed — and keeps the answer in one comment on #38; the run fails the day a feed appears, and fails rather than reporting "none" if it cannot read either source. That comment names the date of the next check. A check that stops does not fail, and GitHub disables a public repository's scheduled workflows after 60 days without activity, so the `contracts` workflow runs `check-sequencer-feed.mjs --watchdog` on every push and pull request and fails while the weekly workflow is disabled or has not run for eight days; `gh workflow enable sequencer-feed.yml` restores it. **Who wires the feed is an open decision.** `setSequencerUptimeFeed` is `onlyOwner` while `setPriceFeed` and `setPriceGuard` sit behind the timelock. A wrong address can only make the pool refuse prices, never accept a bad one, but it can stop every asset at once, which is otherwise the guardian's brake with a designed way back. Whether wiring it waits for the timelock is to be settled when a feed exists to wire. `Deploy.s.sol` wires the feed at deploy when `SEQUENCER_UPTIME_FEED` and `SEQUENCER_GRACE_PERIOD` are both set, and refuses an address that is not a status feed: a price feed pasted there by mistake would answer something other than `0` or `1`, and every price would read as "sequencer down". Chainlink's own example uses a one-hour grace period.
...
`reserve` holds stable against exactly this. It is funded by `reserveFeeShareBps` of every origination fee, topped up by anyone through `fundReserve`, withdrawable only through the timelock and only up to what it holds. Emptying it touches no deposit, but it decides who carries the next shortfall, so a provider gets the same notice of it as of a new LTV. It sits in the pool's balance but is excluded from available liquidity, the same way protocol fees are: neither lendable nor withdrawable by providers.
...
**When the pool is too small.** `liquidate` and `absorbBadDebt` refuse, with `pool too small`, only a cancellation that would take provider deposits to zero: the product-sum accounting cannot represent an emptied pool, because `P` would reach zero and every later deposit would compound to nothing. The check is on what providers actually carry — the debt cancelled, less whatever the reserve pays — not on the debt itself. Providers can only withdraw available liquidity, so their deposits never fall below the debt outstanding, and the only place the refusal can bind is the boundary where a lone borrower's debt equals everything they have left. There, a shortfall the reserve covers goes through. What is still refused is a cancellation with nothing from the reserve behind it, and it is not a trap: one more unit of deposits clears it, as does one unit of reserve when there is a shortfall for it to pay.
...
**The multisig owns the contracts.** It appoints the guardian, unpauses, collects fees, and sets the price updater. None of those change what an existing position is worth, or who carries a loss.

**The timelock holds the risk parameters.** LTVs, liquidation thresholds, caps, the global ceiling, the position floor, the liquidity buffer, price guards, feed swaps, fees, the liquidation incentive, the reserve share, withdrawals from the reserve, and the desk's auditor all carry `onlyTimelock`. The multisig **cannot reach them directly** even though it owns the contracts; it has to queue a change, wait, and execute. That is enforced onchain rather than by convention.
...
189 tests: unit coverage for fees, LTV and freshness guards, liquidation gain and loss math, partnership settlement, passports, and Chainlink pricing; a partnership default suite covering the reporting deadline, recovery after a silent operator and the forfeited operator share; an unactivated partnership suite covering recovery after a silent owner, the exit's boundary at the funding deadline, the owner's remaining choice to activate, and the exit staying open while paused; a passport operations suite covering the five checks, per-check expiry and withdrawal, and the gate; a timelock suite covering the delay and its boundary, expiry, cancellation, the floor on the delay, and the line between what waits and what does not, the reserve included; a bad debt suite covering gap-down liquidation, the reserve absorbing a shortfall, the reserve running dry with the remainder socialised, the reserve pulled ahead of a gap-down, and the underwater dust write-off, each ending with a solvency assertion; a pool boundary suite covering the lone borrower whose debt equals the deposits, the reserve clearing the refusals there, and the one refusal that remains; a risk caps suite that tests every cap boundary from both sides and holds the accumulators to the positions they summarise; a repayment floor suite covering the repayment that cures an unhealthy position, the allowance and balance an escalation to the whole debt needs, and a floor raised under an open position; a fee basis suite covering the issue's liquidation after a repayment, self-liquidation against a direct close, the fee paid on principal as it goes back, the quote an interface approves against, and principal never exceeding debt; an emergency pause suite that drives every user-facing entry point through all eight combinations of the pool's pause bits and holds the role split and the exits open in each; a dedicated oracle safety suite covering sequencer down, the grace window and its boundary, out-of-band prices, sudden jumps between rounds, a reverting feed, per-asset staleness, and the exits staying open through all of it; plus a handler-based invariant suite that drives randomized action sequences — funding and withdrawing the reserve, the dust write-off, collateral withdrawal and fee collection included, with a position floor set — and holds exact USDC conservation, compounded-deposit consistency, collateral solvency, the accumulators matching the positions, fees and reserve always backed by stable the pool holds, principal never exceeding the debt it is part of, and the P multiplier band.
